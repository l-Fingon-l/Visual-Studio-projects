...
bool repack_replay(const char* replay_name, const char* converted_name, uint32_t patch, uint16_t build)
{
...
#pragma region ReadReplay
    ...
    {
        cout << "This is either not a Warcraft III (" << patch << ") replay file or it is corrupted!\n";
        return false;
    }
    if (replay_header.patch_v != patch)
    {
        cout << "Replay " << replay_name << " does not belong to a patch you wanna switch (" << patch << ")!\n";
        return false;
    }
...
#pragma region ModifyReplay
    ...
    {
        cout << "A mistake of decompression in the " << replay_name << ".\n";
        return false;
    }
...
int main()
{
    ...
    if (!repack_replay(replay_name, replay_name_converted, patch, build))
    {
        cout << "Error in repacking of " << replay_name << ".. Skipped.\n";
    }
