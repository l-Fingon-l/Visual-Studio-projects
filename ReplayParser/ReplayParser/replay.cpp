...
"(8)Deadlock_LV.w3x",
"ce96a732bc3461c508c6BC0263F863DBF94881FA",
"(4)losttemple.w3m"
};
...
bool modifyEncodedString(BYTE* EncStr, int& length)
{
    ...
    for (int i(0); i < 49; i++)
    {
        if (to_low_register((string)MapNames[2 * i + 1]) == to_low_register(name))
        {
            hash = to_low_register((string)MapNames[2 * i]);
