...
// This build: 2020.02.26 00:23 a.m. ////////////////////
/////////////////////////////////////////////////////////
...
void EloChange(double &Elo1, double &Elo2, const int& Games1, const int& Games2, bool winner)
{
	double E1 = 1 / (1 + pow(10.0, (Elo2 - Elo1) / 400));
	double E2 = 1 / (1 + pow(10.0, (Elo1 - Elo2) / 400));

	Elo1 = Elo1 + Calculate_K(Elo1, Games1) * (!winner - E1);
	Elo2 = Elo2 + Calculate_K(Elo2, Games2) * (winner - E2);
}
...
#pragma region CalculatingTheLine
		for (int rounds(score1 + score2); rounds > 0; rounds--)
		{
			while (score1)
			{
				EloChange(Elo[ID1], Elo[ID2], (Games[ID1] + Games[ID1 + amount]), (Games[ID2] + Games[ID2 + amount]), false);
				Times[ID1] = gettime();
				Times[ID2] = gettime();
				Games[ID1]++;
				Games[ID2 + amount]++;
			...
			while (score2)
			{
				EloChange(Elo[ID1], Elo[ID2], (Games[ID1] + Games[ID1 + amount]), (Games[ID2] + Games[ID2 + amount]), true);
				Times[ID1] = gettime();
				Times[ID2] = gettime();
				Games[ID2]++;
				Games[ID1 + amount]++;
...
#pragma region UpdatingTheDataFile
	...
	write << "report = " << report << endl << "K = " << K[0] << " " << K[1] << " " << K[2] << endl;
